...
class RationalCTool(HydroToolInterface, LayerSelectionMixin):
    ...
    def create_gui(self, parent_widget: QWidget) -> QWidget:
        ...
        self.lookup_selector = FileSelector(
            ...
            default_path=r"E:\CLAUDE_Workspace\Claude\Report_Files\Templates\Global References"
        )
        ...
        self.output_selector = DirectorySelector(
            "Output Directory",
            default_path=r"E:\CLAUDE_Workspace\Claude\Report_Files\Projects\2025_Project_Crooked\04_Analysis"
        )
    ...
    def run(self, progress_callback: Optional[Callable[[int, str], None]] = None) -> bool:
        ...
        try:
            ...
            # Second intersection: catchments with land use/soils
    ...
    def calculate_composite_c(self, intersection_layer: QgsVectorLayer, 
        ...
        for feature in intersection_layer.getFeatures():
            ...
            if soil_group is None:
                # For unrecognized soil groups (e.g., Water), use default C=0.95
                c_value = 0.95
                ...
                if c_key not in self.lookup_data:
                    self.progress_logger.log(
                        f"Warning: No C value found for land use '{landuse_code}' and soil group '{soil_group}'", 
                        "warning"
                    )
                    continue
                c_value = self.lookup_data[c_key]
            
            # Accumulate data for catchment
            if catchment_id not in catchment_data:
                catchment_data[catchment_id] = {
                    'total_area': 0.0,
                    'c_area_sum': 0.0,
                    'details': []
                }
            ...
            detail_record = {
                ...
                'soil_group': soil_group or 'N/A',
                'soil_group_original': soil_group_raw,
                'area_acres': area_acres,
                'c_value': c_value,
                'c_area_product': c_value * area_acres
            }
    ...
    def parse_soil_group(self, soil_group_raw: str) -> Optional[str]:
        """Parse soil group, handling split HSGs and unrecognized groups"""
        ...
        if soil_group in ['A', 'B', 'C', 'D']:
            ...
            # For invalid soil groups (Water, etc.), return None to trigger default C=0.95
            return None
    ...
    def create_outputs(self, catchment_layer: QgsVectorLayer, results: Dict, 
        ...
        # Add features with calculated C values
        output_features = []
    ...
    def save_detailed_csv(self, detailed_records: list, catchment_data: dict, output_dir: str):
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'Catchment_ID', 'Landuse_Code', 'Soil_Group', 'Soil_Group_Original',
            ...
            for record in detailed_records:
                writer.writerow([
                    ...
                    round(record['area_acres'], 4),
                    round(record['c_value'], 3),
                    round(record['c_area_product'], 4)
                ])
        ...
        with open(summary_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for catchment_id, data in catchment_data.items():
                if data['total_area'] > 0:
                    ...
                    c_area_sum = round(data['c_area_sum'], 3)
                    writer.writerow([catchment_id, area_acres, c_area_sum, c_comp])
...
📁 Output Files:
• catchments_with_c_value.shp - Shapefile with C_Comp field
• c_value_calculations_detailed.csv - Detailed calculations
• c_value_summary.csv - Summary table
